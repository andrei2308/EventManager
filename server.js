...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error during login: " + error.message });
  }
});
/**
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error during registration: " + error.message });
  }
});
...
app.get("/user", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error fetching user data: " + error.message });
  }
});
...
app.get("/events", authenticateToken, (req, res) => {
  ...
  Event.find({
    ...
    .catch((error) => {
      res
        .status(500)
        .json({ message: "Server error fetching events: " + error.message });
    });
});
...
app.post("/events", authenticateToken, async (req, res) => {
  const { name, description, start_time, end_time, access_code, group } =
    req.body;
  if (group === undefined) {
    ...
    res.json({
      message: "Event created successfully. Your event id is: " + eventId,
      eventId,
    });
  } else {
    const newEvent = new Event({
...
app.get("/events/:eventId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Server error fetching event details: " + error.message,
      });
  }
});
...
app.post("/events/:eventId/join-guest", async (req, res) => {
  ...
  if (!name) {
    return res
      .status(400)
      .json({ message: "Name is required for guest access." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Server error joining event as a guest: " + error.message,
      });
  }
});
app.post("/events/:eventId/join", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error joining event: " + error.message });
  }
});
  ...
  async (req, res) => {
    ...
    try {
      ...
      const participants = users.map((user) => {
        const joinedAt = participantsData.find(
      ...
      res
        .status(500)
        .json({
          message: "Server error fetching participants: " + error.message,
        });
    }
  },
);
...
app.delete("/events/:eventId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error deleting event: " + error.message });
  }
});
app.post("/group", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({
        message: "Group created successfully. Your group id is: " + newGroup.id,
      });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Server error creating group: " + error.message });
  }
});
app.get("/groups", authenticateToken, async (req, res) => {
  ...
  EventGroup.find({ organizer: { $ne: userID } })
    ...
    .catch((error) => {
      res
        .status(500)
        .json({ message: "Server error fetching groups: " + error.message });
    });
});
app.get("/groups/details/:groupId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Server error fetching group details: " + error.message,
      });
  }
});
app.get("/groups/admin/:userId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error fetching user groups: " + error.message });
  }
});
app.get("/events/admin/:userId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error fetching user events: " + error.message });
  }
});
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({
          message: "Server error fetching group events: " + error.message,
        });
    }
  },
);
...
app.get("/events/attended/:userId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Server error fetching attended events: " + error.message,
      });
  }
});
/**
